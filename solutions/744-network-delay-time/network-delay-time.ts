function networkDelayTime(times: number[][], n: number, k: number): number {
    ...
    const minPQ = new CustomMinPriorityQueue<number>(); // val: node (number), prio: weight (number)
    minPQ.push(k, 0);
    ...
    while(minPQ.length > 0) {
        ...
        const { val: currNode, prio: currTime } = minPQ.pop();

        if(distances.has(currNode)) {
            continue;
        }

        distances.set(currNode, currTime);
        minTime = currTime;
        if(distances.size === n) {
            break; // reached all nodes (remaining items in the priority queue can be skipped)
        }

        for(const [neighbor, neighborTime] of adjList.get(currNode)) {
            if(!distances.has(neighbor)) {
                minPQ.push(neighbor, currTime + neighborTime);
            }
        }
    ...
    if(distances.size === n) {
        ...
        return -1; // not all nodes were reached
    }
};

function networkDelayTime(times: number[][], n: number, k: number): number {
    ...
    const naivePQ: [number, number][] = [];
    naivePQ.push([k, 0]);

    let minTime = 0;
    const visited = new Set<number>();
    while(naivePQ.length > 0) {

        naivePQ.sort((a, b) => a[1] - b[1]);

        const [ currNode, currCost ] = naivePQ.shift();
        ...
        for(const [neighbor, neighborCost] of adjList.get(currNode)) {
            if(!visited.has(neighbor)) {
                naivePQ.push([neighbor, currCost + neighborCost]);
            }
        }
    }

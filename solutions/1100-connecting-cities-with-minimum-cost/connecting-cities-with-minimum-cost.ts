function minimumCost(n: number, connections: number[][]): number {
    // Create a weigthed Adjacency List
    ...
    // Choose a node, mark it visited and push it's neighbors into minPQ
    const visited = new Set<number>();
    visited.add(1);
    const minPQ = new CustomMinPriorityQueue<[number, number]>(); // val:[src, dst], prio: weight
    for(const neighbor of adjList.get(1)) {
        const [neighborNode, weight] = neighbor;
        minPQ.push([1, neighborNode], weight);
    }
    ...
    return -1; // can't connect the nodes

function carPooling(trips: number[][], capacity: number): boolean {
    ...
    const minPQ = new CustomMinPriorityQueue<number>(); // val: 'capacity' prio: 'to'

    let currCap = 0;
    for(let i = 0; i < trips.length; i += 1) {
        const [cap, from, to] = trips[i];
        currCap += cap;
        // check priority queue (if current 'from' is greater than top 'to')
        while(minPQ.length > 0 && from >= minPQ.top()) {
            const { val: prevCap, prio: prevTo } = minPQ.pop();
            currCap -= prevCap;
        }
        minPQ.push(cap, to);
        if(currCap > capacity) {

function carPooling(trips: number[][], capacity: number): boolean {
    ...
    const minPQ = new CustomMinPriorityQueue<number>(); // val: numPassengers , prio: to

    let currCap = 0;
    for(let i = 0; i < trips.length; i += 1) {

        const [numPass, from, to] = trips[i];
        currCap += numPass;
        minPQ.push(numPass, to);
        while(minPQ.length > 0 && from >= minPQ.top()) {
            const { val: prevPassengers } = minPQ.pop();
            currCap -= prevPassengers;
        }
        if(currCap > capacity) {
            return false;
        }
